								...
								)
{
	...
	tx.push_back(reg); // MSB == 0 is for writing. LSB is not used in address. Datasheet section 8.1.2.3.
	tx.push_back(value);
	SPITransfer(tx);
...
{
	std::vector<uint8_t> tx;
	tx.push_back(0x80 | reg); // MSB == 1 is for reading. LSB is not used in address. Datasheet section 8.1.2.3.
